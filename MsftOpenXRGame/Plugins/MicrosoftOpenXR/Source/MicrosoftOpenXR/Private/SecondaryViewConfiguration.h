...
namespace MicrosoftOpenXR
{
	class FSecondaryViewConfigurationPlugin : public IOpenXRExtensionPlugin
	{
		...
		{
			SingleProjectionLayer(
				XrViewConfigurationType InViewConfigType, XrSpace InSpace, TArray<XrCompositionLayerProjectionView> InViews)
				: ViewConfigType(InViewConfigType), Space(InSpace), Views(std::move(InViews))
			{
			}
			...
			void Finalize(XrSecondaryViewConfigurationLayerInfoMSFT* SecondaryViewInfo)
			{
				...
				*SecondaryViewInfo = {XR_TYPE_SECONDARY_VIEW_CONFIGURATION_LAYER_INFO_MSFT, nullptr, ViewConfigType,
					XR_ENVIRONMENT_BLEND_MODE_ALPHA_BLEND /* TODO: Fix blend mode */, (uint32_t) LayerPtrs.Num(),
					LayerPtrs.GetData()};
			}
	...
	private:
		...
		// View configs that have been enabled on xrBeginSession and their cached view properties.
